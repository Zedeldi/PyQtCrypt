...
FALLBACK_ICON = os.path.abspath(
	os.path.join(BUNDLE_DIR, "assets", "briefcase.svg")
)
...
def shred_file(filepath, iterations):
	...
	try:
		...
		os.rename(
			filepath,
			os.path.join(os.path.dirname(filepath), random_filename)
		)
...
class SettingsDialog(QDialog):
	def __init__(self):
		...
		self.setWindowIcon(
			QIcon.fromTheme("preferences-system", QIcon(FALLBACK_ICON))
		)
		...
		self.spinbox_shred_iterations.valueChanged.connect(
			self.read_shred_iterations
		)
...
class PyQtCrypt(QSystemTrayIcon):
	...
	def write_zip_backup(self):
		with ZipFile(f"{self.CRYPT_DIR}.old.zip", 'w') as archive:
			for path, filename in list_files(self.CRYPT_DIR):
				full_path = os.path.join(
					os.path.relpath(self.CRYPT_DIR),
					path,
					filename
				)
